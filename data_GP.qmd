...
![Présentation OCDE](OCDE_logo.jpg)(https://www.hatvp.fr/wordpress/wp-content/uploads/2015/12/Logo_OCDE.jpg)
...
#depuis 1990 nous pouvons observer la tendance de l'adopation d'energie renouvelable et les pays qui sont les miteurs de ces changements. 
...
```{r}
# Assuming your data frame is named OCDE_pays
data_eau <- OCDE_pays
...
# Gather the data to have one row per combination of Pays, Variable, and année
data_long <- data_eau %>%
  gather(key = "année", value = "valeur", -Pays, -Variable)

# Convert the "valeur" column to numeric
data_long$valeur <- as.numeric(data_long$valeur)

# List of specific variables
variables_to_include <- c(
  "Stress_hydrique_total_des_prélèvements_d_eau_douce_en_pourcentage_des_ressources_internes_renouvelables",
  "Retraits_annuels_d_eau_douce_total_pourcentage_ressources_internes",
  "Retraits_annuels_d_eau_douce_total_milliards_de_metres_cubes",
  "Prélèvements_bruts_d_eau_douce_par_habitant"
)

# Use the function group_by() to group the data by Variable
results <- data_long %>%
  filter(Variable %in% variables_to_include) %>%
  group_by(Variable) %>%
  summarise(
    mean_value = mean(valeur, na.rm = TRUE),
    min_value = ifelse(all(is.na(valeur)), NA, min(valeur, na.rm = TRUE)),
    max_value = ifelse(all(is.na(valeur)), NA, max(valeur, na.rm = TRUE))
  ) %>%
  mutate(across(starts_with("value"), ~ ifelse(is.finite(.), round(., 2), .)))
