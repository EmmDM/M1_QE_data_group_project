...
### **Insertion du Logo de la World bank et de l'OCDE**
...
![Présentation OCDE](OCDE_logo.jpg)**Here the links to the original sources : For the OECD :** https://stats.oecd.org/index.aspx?lang=fr&datasetcode=green_growth
...
![Présenation de la World Bank](world_bank_logo.jpg) **Here the link for the World Bank :** https://donnees.banquemondiale.org/theme/changement-climatique

## **Question de recherche**

Dans la quête d'un développement durable et d'un équilibre harmonieux entre croissance économique et préservation de l'environnement, l'analyse des tendances de transition écologique parmi différents groupes de pays devient un domaine d'investigation crucial. 

Dès lors, le cadre de notre recherche vise à examiner la dynamique environnementale et économique des pays sélectionnés couvrant différentes régions et affiliations en considérant les pays membres et non membres de l'Organisation de Coopération et de Développement Économiques (OCDE).

La période d'intérêt que nous considérons couvre les années 1990 à 2020, permettant un examen complet des tendances à long terme et des développements récents.  De plus, l'OCDE en tant que consortium de pays économiquement avancés, devrait présenter certains modèles de transition écologique qui pourraient différer de ceux observés dans les pays non membres de l'OCDE. En examinant les tendances au sein et entre ces groupes, nous chercherons à démêler l'interaction complexe des facteurs économiques, environnementaux et politiques qui ont façonné leurs trajectoires respectives. Ainsi, la question de recherche qui guidera notre étude et à laquelle nous tâcherons de répondre est la suivante :

*Comment la transition écologique a-t-elle évoluée entre 1990 et 2020 pour les groupes de pays de l'OCDE et ceux hors de cette organisation?*
...
knitr :: kable(info_OCDE_variable, caption = "Nombre de variables de l'OCDE sélectionnées")
knitr :: kable(info_Worldbank_variable, caption = "Nombre de variable de la World Bank selectionnées")
```
...
| Pourcentage de la population exposée aux inondations fluviales| Part de la population résidant dans des zones exposées aux inondations fluviales.  |
| Pourcentage de la population exposée aux incendies incontrôlés | Indique la proportion de la population qui pourrait être impactée par des incendies incontrôlés, fournissant une mesure de la vulnérabilité de la population face à ce risque spécifique.|
>>>>>>> 419abd793391dc5858cec6d7c49a30dd9b619a76
...
knitr :: kable(OCDE_count_ry, caption = "Nombre de pays dans la base de données l'OCDE après nettoyage")
knitr :: kable(WB_count_ry, caption = "Nombre de pays dans la base de données de la World Bank après nettoyage")
```
...
#Dans cette deuxième partie, l'objectif est d'étudier les pressions environnementales sur les ressources exercées par les composantes du PIb, puis les politiques mises en place pour réduire cela. 
...
```{r}
année <- summary(OCDE_pays [c("1990","1991","1992","1993","1994","1995","1996","1997","1998","1999","2000","2001","2002","2003","2004","2005","2006","2007","2008","2009","2010","2011","2012","2013","2014","2015","2016","2017","2018","2019","2020")])


ggplot(OCDE_pays, aes(x = valeur, y = année)) +
  geom_curve(aes(x = valeur, y = année), col = 'Pays')


ggplot(OCDE_pays, aes(x = valeur, y = année))+
  geom_curve(colors = 'Pays')

```
```{r}
# Assuming "année" and "valeur" are your vectors
length_annee <- length(année)
length_valeur <- length(valeur)

if (length_annee == length_valeur) {
  print("Les vecteurs ont la même taille.")
} else {
...
# Assuming "année" and "valeur" are your vectors
max_length <- max(length(année), length(valeur))
année <- c(année, rep(NA, max_length - length(année)))
valeur <- c(valeur, rep(NA, max_length - length(valeur)))

```


```{r}
cols<- c("Prélèvements bruts d'eau douce par habitant",
  "Stress hydrique, total des prélèvements d'eau douce en % des ressources internes renouvelables",
"Retraits annuels d’eau douce, total (% des ressources internes)",
  "Retraits annuels d’eau douce, total (milliards de mètres cubes)")
valeur<-summary(OCDE_pays[c('Variable')])
```
...
#L'industrialisation intensives qui a lieu depuis le 19 ieme siècle et la consommation de masse des 30 glorieuses n'a pas laissé notre planète indemne. En effet, les catastrophes naturelles se multiplient et nous sommes totalement vulnérable face à cet nature déchainée. 
