...
```{r}
#pour les pays de l'OCDE
# Il faut utiliser pivot_longer pour transformer les colonnes années en une colonne Année
...
ggplot(OCDE_pays_long, aes(x = Année, y = Value)) +
  ...
  labs(title = "Recettes provenant des taxes liées à l'énergie, % recettes des taxes environnementales",
       ...
       y = "Valeur") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) #Ajustement du texte pour qu'il soit plus lisibles


#pour les pays hors OCDE

non_OCDE_pays_long <- non_OCDE_pays %>%
  pivot_longer(cols = c(`1990`, `1991`, `1992`, `1993`, `1994`, `1995`, `1996`, `1997`, `1998`, `1999`, `2000`, `2001`, `2002`, `2003`, `2004`, `2005`, `2006`, `2007`, `2008`, `2009`, `2010`, `2011`, `2012`, `2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`),
               names_to = "Année",
               values_to = "Value") %>%
  mutate(Value = as.numeric(Value))
...
ggplot(non_OCDE_pays_long, aes(x = Année, y = Value)) +
  geom_line(na.rm = TRUE) +
  labs(title = "Recettes provenant des taxes liées à l'énergie, % recettes des taxes environnementales",
       x = "Année",
       y = "Valeur") +
...
```
#Voici deux grapsh presentant les Recettes provenant des taxes liées à l'énergie pour les pays OCDE et ceux hors OCDE. Leurs croissance exponentielle à commencée dans les années 2004-2005 pour les pays OCDE et hors OCDE. 
Les taxes representent une mesure efficace à moindre coût, les pays dans l'OCDE souvent plus developpés que la moyenne hors OCDE ont commencés à se doter de cadres d'actions nationaux afin de préserver notre environnement.
...
# Filtrer les données pour les deux variables d'intérêt
selected_data <- c(
  "Développement de technologies liées à l'environnement, % inventions dans le monde",
  "Développement de technologies liées à l'environnement, inventions par habitant"
)
...
# Créer le graphique avec deux courbes
ggplot(selected_data, aes(x = as.numeric(Année), y = Value, color = Variable, linetype = Variable)) +
  geom_line() +
  labs(title = "Développement de technologies liées à l'environnement",
       x = "Année",
       y = "Valeur") +
  theme_minimal()
