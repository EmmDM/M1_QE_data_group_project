...
Les graphiques présentant le lien entre taux de croissance du PIB et taux de croissance des émissions GES ainsi que les tableaux de régression nous suggère une relation positive significative entre le taux de croissance du PIB et le taux de croissance des émissions de GES pour les deux groupes de pays (OCDE et non-OCDE). Dans le cas du groupe OCDE, chaque augmentation d'une unité dans le taux de croissance du PIB est associée à une augmentation d'environ 1,0843 unité dans le taux de croissance des émissions de GES. Tandis que dans le cas du groupe non-OCDE, chaque augmentation d'une unité dans le taux de croissance du PIB est associée à une augmentation d'environ 0.83193 unité dans le taux de croissance des émissions de GES. De plus, les R-squared élevés indiquent que les modèles expliquent une grande partie de la variabilité observée.

Ainsi, cela nous amène à nous interesser à différents indicateurs qui nous permettent de mieux comprendre l'évolution conjointe des émissions de GES et du PIB des différents pays qui constituent notre panel.
...
Bulgarie,...) et relatif (	
Uruguay,Chine,...). La catégorie "Autre" regroupe des pays dont le taux d'émission de GES ou taux de PIB sont manquants.

## B. Courbe environnementale de Kuznets

Dans un second temps nous nous intéressons à la courbe environnementale de Kuznets, qui selon l'économiste Éloi Laurent, spécialiste de la croissance, considère que la source de l'idée de découplage repose sur la courbe environnementale de Kuznets.
Initiallement, la courbe de Kuznets, dont la forme est U inversé, représente l'inégalité économique dans un pays en fonction de son niveau de développement. C'est en 1994 que Grossman et Krueger postulent le modèle de courbe environnementale de Kuznets (EKC) qui donne une relation déterministe entre développement économique et qualité de l'environnement. Dans la première étape de l'industrialisation, la pollution augmente rapidement car les individus sont principalement interessées par leur travail et leur revenus que par la qualité de l'air et de l'eau. A ce stade, la société est trop pauvre pour réduire ses émissions, de fait la réglementation environnementales
est trop failbe. Le retournement s'effectue à mesure que les revenus augmentent. Dès lors, les principaux secteurs industriels deviennent plus propres, les individus valorisent l'environnement et les institutions de régulation deviennent plus efficaces. Le long de la
courbe, la pollution se stabilise dans la tranche des revenus moyens, puis retombe au niveau pré-industriel dans les sociétés développées. Ainsi, la CKE est une théorie intéressante qui nous permet de mieux comprendre comment une transition écologique peut avoir lieu à mesure que les pays se développe. 
...
CKE_1990_graph <- ggplot(CKE_1990, aes(x = PIB_hab, y = GES_hab, color = Group)) + geom_point(na.rm = TRUE) + geom_smooth(method = 'lm', formula = y ~ poly(x, 2), se = FALSE, color = 'darkorange') + labs(title = "CKE en 1990", x = "PIB par habitant", y = "Emmission de GES")
...
CKE_2020_graph <- ggplot(CKE_2020, aes(x = PIB_hab, y = GES_hab, color = Group)) + geom_point(na.rm = TRUE) + geom_smooth(method = 'lm', formula = y ~ poly(x, 2), se = FALSE, color = 'darkorange') + labs(title = "CKE en 2020", x = "PIB par habitant", y = "Emmission de GES")


(CKE_1990_graph  | CKE_2020_graph ) + plot_annotation(title = "Les courbes de Kuznets environnementales (CKE) en 1990 et 2020")
```
