---
title: "Final Part (suite de data_GP.qmd)"
...
présentation nous nous focaliserons sur les émissions totales de gaz à effet
...
Les pays dont le point se trouve à gauche de l'axe des ordonnées ou au-dessus de la droite d'équation y = x sont des pays où le découplage n'a
...
En utilisant les données des pays pour les émissions de GES et le PIB
...
diminué les émissions de GES, comme le montre le graphique sur le lien entre taux de croissance du PIB et taux de croissance des émissions GES. 
...
Finalement, nos résultats nous montrent que le découplage économique-environnemental est en cours, mais des différences existent entre les groupes. Toutefois, la CEK ne montre pas de preuves claires de la relation en U inversé entre le développement économique et les émissions de GES, il existe donc des limites au découplage entre croissance économique et décroissance des émissions de GES. Toutefois, l'identité de Kaya met en évidence un certain succès des pays de l'OCDE dans la réduction des émissions. Ces constats soulignent l'importance des politiques environnementales et des initiatives à promouvoir une croissance durable.
...
telles que les prélèvements bruts d'eau douce par habitant (Mètres cubes par habitant), le stress hydrique, les retraits annuels d'eau douce en pourcentage des ressources internes renouvelables, ainsi que le volume total. Ces indicateurs offrent un aperçu des impacts et des utilisations intensives de l'eau,une ressource cruciale confrontée à des défis mondiaux.
...
knitr::kable(eau_stat_OCDE_head, caption = "Intensité de l'eau des pays membres de OCDE")
eau_stat_non_OCDE_head <- head(eau_stat_non_OCDE, 10)
knitr::kable(eau_stat_non_OCDE_head, caption = "Intensité de l'eau des pays non membres OCDE")
...
Selon le scénario de référence des Perspectives à l'horizon 2050 de l'OCDE, 40 %
...
graph_reg_mortalite_PM25_OCDE <- ggplot(reg_mortalite_PM25_OCDE, aes(x = `Croissance de la population urbaine (% annuel)`,y = taux_mortalite_PM25)) + geom_point(color='gray', size=1, na.rm = TRUE) + geom_smooth(method='lm', formula = 'y ~ x', se=FALSE, color='red', na.rm = TRUE) + theme_minimal() + labs(title = "Correlation entre le taux de mortalité due à l'exposition au PM2.5 \n par rapport au taux de croissance de la population urbaine (%) \n des pays membres de l'OCDE", x = "Croissance de la population urbaine (% annuel)", y = "taux de Mortalité due \n à l'exposition aux PM2.5")
...
graph_reg_mortalite_PM25_non_OCDE_taux <-ggplot(reg_mortalite_PM25_non_OCDE_taux, aes(x = `Croissance de la population urbaine (% annuel)`,y = taux_mortalite_PM25_non_OCDE)) + geom_point(color='gray', size=1, na.rm = TRUE) + geom_smooth(method='lm', formula = 'y ~ x', se=FALSE, color='red', na.rm = TRUE) + theme_minimal() + labs(title = "Correlation entre le taux de mortalité due à l'exposition au PM2.5 \n par rapport au taux de croissance de la population urbaine (%) \n des pays non membres de l'OCDE", x = "Croissance de la population urbaine (% annuel)", y = "taux de Mortalité due \n à l'exposition aux PM2.5")

graph_reg_mortalite_PM25_OCDE/graph_reg_mortalite_PM25_non_OCDE_taux
```
...
graphreg_Dévelop_recette_non_OCDE <- ggplot(reg_Dévelop_recette_non_OCDE, aes(x = `Développement de technologies liées à l'environnement, % inventions dans le monde`,y = `Recettes provenant des taxes liées à l'énergie, % recettes des taxes environnementales`)) + geom_point(color='gray', size=1, na.rm = TRUE) + geom_smooth(method='lm', formula = 'y ~ x', se=FALSE, color='red', na.rm = TRUE) + theme_minimal() + labs(title = "Correlation entre le développement de technologies liées à l'environnement \n et les recettes provenant des taxes liées à l'énergie - non OCDE", x = "Développement de technologies liées à l'environnement, % inventions dans le monde", y = "Recettes provenant des \n taxes liées à l'énergie, \n % recettes des taxes \n environnementales")

graph_reg_Dévelop_recette_OCDE/ graphreg_Dévelop_recette_non_OCDE
```
...
graph_subvention <- ggplot(merged_subvention, aes(x = as.numeric(Année), y = Value, color = Groupe)) +
  geom_smooth(method = "loess", se = FALSE, na.rm = TRUE)+
  labs(title = "Soutien total aux combustibles fossiles,\n % recettes totales des taxes",
       x = "Année",
       y = "Recettes en %") +
  theme_minimal()
...
graph_subvention_consommateur <- ggplot(merged_data_subvention_consommateur, aes(x = as.numeric(Année), y = Value, color = Groupe)) +
  geom_smooth(method = "loess", se = FALSE, na.rm = TRUE)+
  labs(title = "Soutien aux consommateurs de \n combustibles fossiles, \n % recettes totales des taxes",
       x = "Année",
       y = "Recettes en %") +
  theme_minimal()
...
graph_subvention_electricite <- ggplot(merged_data_subvention_éléctricité, aes(x = as.numeric(Année), y = Value, color = Groupe)) +
  geom_smooth(method = "loess", se = FALSE, na.rm = TRUE)+
...
(graph_subvention | graph_subvention_consommateur)/graph_subvention_electricite
...
subventions. Des solutions alternatives, telle que l'utilisation l'énergie "décarbonée",
doivent être explorées pour atteindre les objectifs climatiques
mondiaux, notamment en raison de la vulnérabilité grandissante de notre société.

# III. Vulnérabilité et stratégies adaptives
...
100%, comme le Koweït dans le groupe hors-OCDE.
...
ggplot(merged_migration, aes(x = as.factor(Année), y = Value, fill = Groupe)) +
  ...
  labs(title = "Évolution de la migration nette par groupe",
       x = "Année",
       y = "Migration Nette (Pour 1 000 habitants)") +
  theme_minimal() +
...
L'analyse de l'immigration nette pour 1000 habitants offre des
aperçu sur la dynamique démographique et migratoire. Les tendances
...
Le tableau de régression entre migration nette et prélèvements bruts d'eau
douce par habitant inique qu'il n'y a pas suffisamment de preuves statistiques pour affirmer avec certitude que cette relation est significative. Malgré un lien entre la hausse de la
...
## B. Vulnérabilité humaine face au changement climatique
...
graph_average_zones_protégées <- ggplot(average_zones_protégées, aes(x = as.factor(Année), y = Average_Value, fill = Groupe)) +
...
graph_average_terrestres_marines <- ggplot(average_terrestres_marines , aes(x = as.factor(Année), y = Average_Value, fill = Groupe)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7, na.rm = TRUE) +
  ...
  theme_minimal()
(graph_average_zones_protégées / graph_average_terrestres_marines)
```
...
Bien que tous les pays n'ont pas été évalué, la moyenne de l'OCDE est de 3.80 et celle du groupe Non-OCDE de 3.14, cela montre un effort global dans la volonté de réduire les risques de catastrophes, toutefois nons n'avons pas plus de données pour déterminer une évolution de cette note. Par ailleurs, selon les rapports du GIEC, la vulnérabilité des pays aux événements climatiques extrêmes devraient augmenter à l'avenir en raison du réchauffement climatique. Les impacts potentiels de cette vulnérabilité accrue pourraient inclure des perturbations majeurs dans les systèmes sociaux, économiques et environnementaux, avec des conséquences sur la sécurité alimentaire, la santé publique et la stabilité sociale. Il est donc essentiel que l'ensemble des pays progresse dans la réduction des risques de catastrophes.

# Conclusion

Pour conclure, nous constatons des disparités marquées entre les pays de l'OCDE et les autres pays dans la transition écologique entre 1990 et 2020.  Bien que certains pays de l'OCDE aient réussi à découpler la croissance économique des
...
impacts du changement climatique se traduisent par une vulnérabilité
...
- Sustainable Development, Indicators to measure decoupling of environmental pressure from economic growth, (2002), OECD

- PELLEGRIS, Alban. Le découplage entre la consommation d’énergie et le PIB : questionnements théoriques et évaluations empiriques du rôle de l’énergie dans le processus de croissance économique - 2022
...
Thomas Jobert, Fatih Karanfil, Dans Innovations 2012/1 (n°37), pages 11 à 26

- Wei Ming Huanga, Grace W.M. Leea, Chih Cheng Wub, (2007), GHG emissions, GDP growth and the Kyoto Protocol:A revisit of Environmental Kuznets Curve hypothesis 
...
- OCDE. Exposition à la pollution de l'air.
<https://data.oecd.org/fr/air/exposition-a-la-pollution-de-l-air.htm#indicator-chart>

- Santé Publique France. Impacts de l'exposition chronique aux
