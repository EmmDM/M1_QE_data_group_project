...
# Filtrer les données pour la variable d'intérêt
selected_data <- OCDE_pays %>%
  filter(Variable == "Recettes provenant des taxes liées à l'énergie, % recettes des taxes environnementales") %>%
  select(Pays, `1990`:`2020`) %>%
  pivot_longer(cols = `1990`:`2020`, names_to = "Année", values_to = "Value") %>%
  mutate(Value = as.numeric(Value))

# Créer le graphique en barres avec des couleurs selon l'intensité
ggplot(selected_data, aes(x = as.numeric(Année), y = Value, fill = Value)) +
  geom_bar(stat = "identity") +
  labs(title = "Recettes provenant des taxes liées à l'énergie, % recettes des taxes environnementales",
       x = "Année",
       y = "Recettes en %") +
...
# Supposons que vos données sont dans un dataframe appelé "OCDE_pays"
modele <- lm(Value ~ Pays, data = OCDE_pays[OCDE_pays$Variable %in% c("Développement de technologies liées à l'environnement, % inventions dans le monde", "Recettes provenant des taxes liées à l'énergie, % recettes des taxes environnementales"),])
