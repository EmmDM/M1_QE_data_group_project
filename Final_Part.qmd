...
*I- Découplage* Dans un premier temps, nous chercherons à présenter un
...
```{r}
#Pour le CO2
#Tous les pays de l'OCDE et est les trois régions principales (Europe, Amérique du Nord et Pacifique)
...
#ggplot(decoupling_CO2_OCDE_index, aes(x = Année, y = Ratio)) + geom_jitter() + labs(title = "Ratio de découplage entre les émissions de CO2 et le PIB Réel par habitant", x = "Année", y = "Ratio de découplage") +  facet_wrap(~ Pays, scales = "free_y", ncol = 5, strip.position = "bottom") + theme(legend.position = "top", legend.title = element_text(size =10), legend.text = element_text(size = 8), axis.text.x = element_text(angle = 45, hjust = 1), axis.text.y = element_text(size= 8), plot.title = element_text(size = 14, face = "bold"), plot.subtitle = element_text(size = 12),plot.caption = element_text(size = 10), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),panel.background = element_rect(fill = "white"), panel.border = element_blank())
