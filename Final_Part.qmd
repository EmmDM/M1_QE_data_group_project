...
ggplot(reg_Dévelop_recette_OCDE, aes(x = `Développement de technologies liées à l'environnement, % inventions dans le monde`,y = `Recettes provenant des taxes liées à l'énergie, % recettes des taxes environnementales`)) + geom_point(color='gray', size=1, na.rm = TRUE) + geom_smooth(method='lm', formula = 'y ~ x', se=FALSE, color='red', na.rm = TRUE) + theme_minimal() + labs(title = "Correlation entre le développement de technologies liées à l'environnement et les recettes provenant des taxes liées à l'énergie", x = "Développement de technologies liées à l'environnement, % inventions dans le monde", y = "Recettes provenant des taxes liées à l'énergie, % recettes des taxes environnementales")
...
ggplot(merged_data_zone_protégée, aes(x = as.factor(Année), y = Value, fill = Groupe)) +
  ...
  labs(title = "Zones protégées à l'échelle nationale (% du territoire total)",
       x = "Année",
       y = "Pourcentages zones protégées") +
  theme_minimal()
