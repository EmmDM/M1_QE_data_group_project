...
Ces indicateurs ne refletent pas la réalité complexe, la diminution de la moyenne annuelle transcrit le fait que les pays de l'OCDE developpés ont finis leurs croissance exponentielles. Cependant les BRIICS et les autres pays emergents devrait voir leurs consommation augementer fortement pour le secteur manufacturier, la production d'éléctricité et les besoins domestiques. On peut apercevoir cet écart de croissance avec le premier tableau qui montre une décroissance des prélevements bruts d'eau douce par habitant en dessous de 400 contrairement aux pays hors OCDE qui voient leurs prélevement croître depuis 2006. 



Le constat que nous poulons faire avec l'aide de ces données est la tendance   stable de l'OCDE depuis les années 1980 en matière des prelevement totaux d'eau de surface mais en parallele les quantitées pour l'irriguation ont augmentés. Ont peux expliquer cela par l'implantation de nouvelles technologies et la transition d'une activitée à une autre. Nous illustrons cela par 4 graph pour les pays de l'oCDE. 
...
ggplot(CO2_emission_habitant_OCDE, aes(x = Année, y = Ratio_base100))  + geom_point() + labs(title = "Emissions CO2 et autres GES(HFC,PFC,SF6) par personne en base 100 (1990) pour le total des pays non membres de l'OCDE", x = "Année", y = "Ratio (Base 100 en 1990)") + theme(axis.text.x=element_text(angle=50, size=8, vjust=0.5)) 
...
#utiliser pop urbaine : plus il y a une pop urbaine grd plus la mortalité augmente 
#x :tx croissance pop
taux mortalité chaque anné : création taux(index) : faire taux de variation / croissance pop deja en taux : corrélation 
#y = taux de mortalité 
...
taux_pop_OCDE <- OCDE_pays %>%
  filter(Variable == "Croissance de la population (% annuel))") %>%
  select(-`COU`, -`VAR`, -`Unit`, -`PowerCode`) %>%
  pivot_longer(cols = c(`1990`, `1991`, `1992`, `1993`, `1994`, `1995`, `1996`, `1997`, `1998`, `1999`, `2000`, `2001`, `2002`, `2003`, `2004`, `2005`, `2006`, `2007`, `2008`, `2009`, `2010`, `2011`, `2012`, `2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`),  
               names_to = "Année", values_to = "Croissance_de_la_population") %>%
  mutate(Croissance_de_la_population = as.numeric(Croissance_de_la_population)) %>%
  pivot_wider(names_from = Variable, values_from = Croissance_de_la_population) 
...
reg_mortalite_PM25_OCDE=lm('taux_mortalite_PM25 ~ `Croissance de la population (% annuel)`', data = mortalité_PM25_pop_OCDE)
summary(reg_mortalite_PM25_OCDE)
ggplot(reg_mortalite_PM25_OCDE, aes(x = `Croissance de la population (% annuel)`,y = taux_mortalite_PM25)) + geom_point(color='gray', size=1, na.rm = TRUE) + geom_smooth(method='lm', formula = 'y ~ x', se=FALSE, color='red', na.rm = TRUE) + theme_minimal() + labs(title = "Correlation entre le taux de mortalité due à l'exposition au PM2.5 par rapport au taux de croissance de la population urbaine (%) ", x = "Croissance de la population (% annuel)", y = "taux de Mortalité due à l'exposition aux PM2.5")
...
```{r}
  ...
  mortalité_PM25_OCDE <-  OCDE_pays %>% filter(Variable %in% c("Mortalité due à l'exposition aux PM2.5","Population, total")) %>% select(-COU, -VAR ,- Unit, -PowerCode) %>%pivot_longer(cols = c(`1990`,`1991`,`1992`,`1993`,`1994`,`1995`,`1996`,`1997`,`1998`,`1999`,`2000`, `2001`, `2002`, `2003`, `2004`, `2005`, `2006`, `2007`, `2008`, `2009`,`2010`, `2011`, `2012`, `2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`),  names_to = "Année", values_to = "Value") %>% mutate(Value = as.numeric(Value)) %>% pivot_wider(names_from = Variable, values_from = Value) 
  

#Création d'un index 

mortalité_pm25_OCDE_index <-  mortalité_PM25_OCDE %>% filter(Pays%in%unique(mortalité_PM25_OCDE$Pays)) %>% group_by(Année) %>%
summarise(mortalité_PM25 = sum(`Mortalité due à l'exposition aux PM2.5`, na.rm = TRUE), Hab_tot = sum(`Population, total`,na.rm = TRUE)) %>% mutate(mortalité_total = `mortalité_PM25`/`Hab_tot`)
  
  
  
  
  
  OCDE_pays_pm25 <- OCDE_pays %>%
  filter(Variable %in%c("Mortalité due à l'exposition aux PM2.5", "Exposition moyenne de la population aux PM2.5")) %>%
  pivot_longer(cols = c(`1990`, `1991`, `1992`, `1993`, `1994`, `1995`, `1996`, `1997`, `1998`, `1999`, `2000`, `2001`, `2002`, `2003`, `2004`, `2005`, `2006`, `2007`, `2008`, `2009`, `2010`, `2011`, `2012`, `2013`, `2014`, `2015`, `2016`, `2017`, `2018`, `2019`, `2020`),
               names_to = "Année",
               values_to = "Value") %>%
  mutate(Value = as.numeric(Value), Groupe = "OCDE")
  
  #unité differente pour les 2 variables comment ajuster cela ??
```


```{r}
ggplot(OCDE_pays_pm25, aes(x = Année, y = Value, color = Variable, linetype = Variable, group = Variable))  + geom_line(na.rm = TRUE) + labs(title = "Taux de croissance du PIB par habitant et taux de croissance des émissions de CO2 des pays membres de l'OCDE", x = "Année", y = "Taux de croissance (%)", color = "Variable") +  theme(axis.text.x=element_text(angle=50, size=8, vjust=0.5))
```
...
ggplot(non_OCDE_corr_wide_convert, aes(x = Année, y = Taux, color = Variable, linetype = Variable)) +
  ...
  labs(title = "Taux de développement de technologies liées à l'environnement et de recettes des taxes environnementales - non OCDE",
       x = "Année",
       y = "Taux") +
...
ggplot(merged_data_zone_protégée, aes(x = as.factor(Année), y = Value, fill = Groupe)) +
...
ggplot(OCDE_zone_protégée_totale, aes(x = as.numeric(Année), y = Value, color = Groupe)) +
  geom_smooth(method = "loess", se = FALSE, na.rm = TRUE)+
  labs(title = "Soutien total aux combustibles fossiles, % recettes totales des taxes",
       x = "Année",
       y = "Recettes en %") +
