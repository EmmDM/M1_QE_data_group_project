...
eau_OCDE <- OCDE_pays %>% filter(Variable, Année, Pays)
...
eau_stat_OCDE_min <- eau_OCDE %>% summarise (
  min_eau = min("Prélèvements bruts d'eau douce par habitant")
)
                                             
knitr :: kable(eau_stat_OCDE_min)


eau_stat_non_OCDE <- eau_non_OCDE %>% group_by(Pays,Variable) %>% summarize(Valeur_moyenne = mean(`Value`, na.rm = TRUE), valeur_median = median(`Value`, na.rm = TRUE), valeur_min = min(`Value`, na.rm = TRUE), valeur_max = max(`Value`, na.rm = TRUE), .groups = 'drop')
...
#1) emission GES et autre
library(dplyr)
library(corrplot)

qualité_air <- OCDE_pays %>%
  filter(Variable %in% c(  "Autres émissions de gaz à effet de serre, HFC, PFC et SF6 (milliers de tonnes métriques d’équivalent CO2)",
                          ...
                          "Émissions totales de GES (kt d’équivalents CO2)", "Mortalité due à l'exposition aux PM2.5","Pourcentage de la population exposée à plus de 10 microgrammes/m3", "Pourcentage de la population exposée à plus de 35 microgrammes/m3",
...
selected_variables <- c("Autres émissions de gaz à effet de serre, HFC, PFC et SF6 (milliers de tonnes métriques d’équivalent CO2)",
                         "Émissions totales de GES (kt d’équivalents CO2)",
...
selected_data <- OCDE_pays %>%
  ...
  select(Année, Pays, Variable, Value)

# Convert "Année" to a factor for proper ordering
selected_data$Année <- factor(selected_data$Année, levels = unique(selected_data$Année))
...
for (Variable in selected_variables) {
  ...
  # Plot for the current variable
  ggplot(variable_data, aes(x = Année, y = Value, fill = Pays)) +
    geom_bar(stat = "identity", position = "dodge") +
    labs(title = Variable,
         x = "Year",
         y = "Value") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
    facet_wrap(~pays, scales = "free_y", ncol = 3)  # Adjust ncol as needed
...
"Autres émissions de gaz à effet de serre, HFC, PFC et SF6 (milliers de tonnes métriques d’équivalent CO2)", "Émissions/éliminations nettes de GES attribuables au changement d'affectation des terres et à la foresterie (Mt d’équivalents CO2)","Émissions totales de GES (kt d’équivalent CO2)") %>%,
Pays 
...
```{r}
air_stat_OCDE <- OCDE_pays %>% group_by(Pays,Variable) %>% summarize(Valeur_moyenne = mean(`Value`, na.rm = TRUE), valeur_median = median(`Value`, na.rm = TRUE), valeur_min = min(`Value`, na.rm = TRUE), valeur_max = max(`Value`, na.rm = TRUE), .groups = 'drop')
air_stat_non_OCDE <- OCDE_pays %>% group_by(Pays,Variable) %>% summarize(Valeur_moyenne = mean(`Value`, na.rm = TRUE), valeur_median = median(`Value`, na.rm = TRUE), valeur_min = min(`Value`, na.rm = TRUE), valeur_max = max(`Value`, na.rm = TRUE), .groups = 'drop')

knitr :: kable(air_stat_OCDE, caption = "Tableau de valeurs moyennes et médianes sur les variables hydriques d'intérêt par pays")

knitr :: kable(air_stat_non_OCDE, caption = "Tableau de valeurs moyennes et médianes sur les variables hydriques d'intérêt par pays")


extreme_air <- OCDE_pays(Variable =character(),
```
